...
function processArticles(isProduction) {
	...
	files.forEach(fileName => {
		const text = fs.readFileSync(`${articlePath.src}/${fileName}`, "utf8");
		...
		fs.writeFileSync(`${articlePath.dist}/${distName}`, html, "utf8");
		articles.push({
			title,
			date,
			name: distName
...
function processCSS() {
	...
	return files.map(fileName => {
		...
		const text = fs.readFileSync(`${sassPath.src}/${fileName}`, "utf8");
		...
		return new Promise(resolve => {
			postcss([precss, autoprefixer, cssnano()])
				...
				.then(result => {
					inlineCSS[[fileName.replace(".scss", "")]] = result.css;
					resolve();
				});
		});
	});
...
module.exports = (isProduction = false) => {
	return Promise.all(processCSS()).then(() => processArticles(isProduction));
};
